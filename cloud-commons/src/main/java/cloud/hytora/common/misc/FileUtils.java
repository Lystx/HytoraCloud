...
public final class FileUtils {
     ...
     * @throws IOException if read fails for any reason
     */
    public static String getResourceFileAsString(InputStream is) throws IOException {
            ...
            for (File currentFile : files) {
                if (currentFile.isDirectory()) {
                    ...
                    if (currentFile.getName().equals("session.lock"))
                        continue; // Don't delete or we'll get lots of exceptions
                    currentFile.delete();
                }
            }
        }
     ...
     * @throws IOException if read fails for any reason
     */
    public static String getResourceFileAsString(String fileName, Class<?> accessor) throws IOException {
        InputStream is = accessor.getResourceAsStream(fileName);
        if (is == null) {
            return null;
        }
    ...
    public static void writeToFile(File file, Supplier<Collection<String>> content) throws IOException {
        writeToFile(file, (content.get()));
    }

    public static void writeToFile(File file, Collection<String> content) throws IOException {
        if (!Files.exists(file.toPath())) {
            FileUtils.createFile(file.toPath());
        }
