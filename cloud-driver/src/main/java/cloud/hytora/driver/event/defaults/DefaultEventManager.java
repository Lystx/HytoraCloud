...
public class DefaultEventManager implements EventManager {
    ...
    public <E extends CloudEvent> E callEvent(@NotNull E event, PublishingType... type) {
        ...
        switch (publishingType) {
            case INTERNAL:
                ...
                for (Class<?> clazz : ClassWalker.walk(event.getClass())) {
                    ...
                    for (RegisteredListener listener : new ArrayList<>(listeners)) {
                        ...
                        if (listener.isIgnoreCancelled() && event instanceof Cancelable && ((Cancelable) event).isCancelled())
                            continue;
                ...
                if (event instanceof Cancelable) {
                    CloudDriver.getInstance().getLogger().debug("=> {}: cancelled={}", event.getClass().getSimpleName(), ((Cancelable) event).isCancelled());
                }
                break;
            case PROTOCOL:
            ...
            case GLOBAL:
                callEvent(event, PublishingType.PROTOCOL);
                callEvent(event, PublishingType.INTERNAL);
                break;
            default:
                break;
        }
