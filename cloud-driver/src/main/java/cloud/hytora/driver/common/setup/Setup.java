...
public abstract class Setup<T extends Setup<?>> {
    ...
    public Screen getSetupScreen() {
        return CloudDriver.getInstance().getProvider(ScreenManager.class).getCachedScreen(this.uniqueSetupName);
    }
    ...
    public void start(SetupListener<T> finishHandler) {
        ...
        Screen screen = screenManager.getCachedScreen(this.uniqueSetupName);
    ...
    public <T> T get(String fieldName) {
        Field field = ReflectionUtils.getInheritedPrivateField(getClass(), fieldName);
    ...
    private void printQuestion(SetupEntry entry) {
        ...
        if (entry.getRequiresEnum() != null) {
            this.getSetupScreen().writeLine("§7Possible Answers§8: %1" + Arrays.toString(entry.getRequiresEnum().value().getEnumConstants()).replace("]", "§8)").replace("[", "§8(%1").replace(",", "§8, %1"));
        }
    ...
    private void printHeader(String header) {
        ...
        this.getSetupScreen().writeLine("§7» §7Current Question §f: %1" + (this.current == 1 ? 1 : current) + "/" + (this.map.keySet().size() == 0 ? "Loading" : this.map.keySet().size() + ""));
        this.getSetupScreen().writeLine("§8");
    }
