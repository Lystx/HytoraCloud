...
import cloud.hytora.driver.common.http.html.HtmlContent;
...
public interface HttpMessage<M extends HttpMessage<?>> {
	...
	default M setContentType(HttpContent contentType) {
		return setHeader("Content-Type", contentType.getHeaderFormat());
	}

	default M setHtml(HtmlContent html) {
		return setBody(html.getContent());
	}
