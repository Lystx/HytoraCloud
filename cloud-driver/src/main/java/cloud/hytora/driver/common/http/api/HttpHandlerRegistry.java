...
public class HttpHandlerRegistry {
	...
	public Collection<RegisteredHandler> getRegisteredHandlers(String path) {
		return handlers.stream().filter(h -> h.getPath().startsWith(path)).collect(Collectors.toList());
	}
}
