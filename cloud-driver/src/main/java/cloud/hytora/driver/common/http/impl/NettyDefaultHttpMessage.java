...
import cloud.hytora.driver.common.http.api.HttpMessage;
import cloud.hytora.driver.common.http.api.HttpVersion;
...
public interface NettyDefaultHttpMessage<M extends HttpMessage<?>> extends HttpMessage<M> {
	...
	default M setBody(@Nonnull String text) {
		if (text == null) {
			Logger.constantInstance().error("Tried setting body of HttpMessage to nulled text");
			return self();
		}
	...
	default M setBody(@Nonnull Collection<Document> array) {
		return setBody(Bundle.newGsonBundle(array).asRawJsonString());
	}
