...
public class BungeeConfiguration extends VersionFile {
    ...
    @Override
    public void applyFile(CloudService service, File file) throws IOException {

        FileWriter writer = new FileWriter(file);

        List<CloudService> services = CloudDriver.getInstance().getServiceManager().getAllServicesByEnvironment(SpecificDriverEnvironment.MINECRAFT);
        CloudService firstService = services.isEmpty() ? null : services.get(0);
