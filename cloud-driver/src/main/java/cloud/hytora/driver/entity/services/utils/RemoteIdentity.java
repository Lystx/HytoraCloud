...
public class RemoteIdentity implements IBufferObject {
    ...
    public void save(File file) {
        try {
            Document.gson(this).saveToFile(file);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public static RemoteIdentity forApplication(ProtocolAddress address) {
        return new RemoteIdentity(address.getAuthKey(), "", VersionType.BUNGEE, ServiceProcessType.WRAPPER, LogLevel.INFO, address.getHost(), CloudDriver.Constants.APPLICATION_NAME, address.getPort());
    }

    public static RemoteIdentity read(File file) {
        return Document.gsonUnchecked(file).toInstance(RemoteIdentity.class);
    }
