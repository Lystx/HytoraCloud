...
@NoArgsConstructor
@Getter
public class DefaultNodeData implements INodeCycleData {
	...
	public void applyBuffer(BufferState state, @NotNull PacketBuffer buf) throws IOException {
		...
		switch (state) {
			...
			case WRITE:
				...
				buf.writeInt(cores);
				buf.writeLong(maxRam);
				buf.writeLong(freeRam);
	...
	public boolean hasTimedOut() {
		...
		int lostCycles = (int) lastCycleDelay / CloudDriver.Constants.NODE_PUBLISH_INTERVAL;
		if (lostCycles > 0) {
			CloudDriver.getInstance().getLogger().trace("Node timeout: lost {} cycles ({}ms)", lostCycles, lastCycleDelay);
		}
		return lostCycles >= CloudDriver.Constants.NODE_MAX_LOST_CYCLES;
	}
