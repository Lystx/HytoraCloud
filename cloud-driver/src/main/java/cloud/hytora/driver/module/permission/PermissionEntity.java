...
public interface PermissionEntity extends IBufferObject {
    ...
    /**
     * Adds a denied permission to this entities permissions
     *
    ...
    /**
     * Removes a denied permission from this entities permissions
     *
     * @param permission the permission to deny
     * @see #getDeniedPermissions()
     */
    ...
    void addTaskPermission(ServiceTask task, String permission);
    void removeTaskPermission(ServiceTask task, String permission);

    /**
     * The permissions that this entity has on a specific task
     */
    Map<ServiceTask, Collection<String>> getTaskPermissions();

    @Nonnull
    default Collection<String> getTaskPermissions(String taskName) {
        Optional<Map.Entry<ServiceTask, Collection<String>>> first = getTaskPermissions().entrySet().stream().filter(e -> e.getKey().getName().equalsIgnoreCase(taskName)).findFirst();
     ...
     */
    void setTaskPermissions(Map<ServiceTask, Collection<String>> taskPermissions);
