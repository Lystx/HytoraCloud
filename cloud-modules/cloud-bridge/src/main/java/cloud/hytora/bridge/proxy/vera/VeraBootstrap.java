...
import cloud.hytora.bridge.proxy.vera.adapter.VeraPlayer;
...
public class VeraBootstrap implements PluginBridge, RemoteProxyAdapter {
    ...
    public Collection<LocalProxyPlayer> getPlayers() {
        return VeraProxy.getInstance().getOnlinePlayers().stream().map(VeraPlayer::new).collect(Collectors.toList());
    }
