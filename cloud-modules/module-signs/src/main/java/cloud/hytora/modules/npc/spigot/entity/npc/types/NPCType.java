...
import cloud.hytora.modules.npc.spigot.entity.cache.TypeCache;
import cloud.hytora.modules.npc.spigot.entity.npc.CustomizationLoader;
...
/**
 * List of supported entity types for a {@link SpigotNPC}.
 */
public enum NPCType {
    ...
    NPCType(Class<?> entityClass,
        ...
        try {
            if (Utils.versionNewer(14)) {
                nmsEntityType = ((Optional<?>) CacheRegistry.ENTITY_TYPES_A_METHOD.load().invoke(null, bukkitEntityType.name().toLowerCase())).get();
     ...
     *
     * @param spigotNpc The npc to update the customization for.
     * @param name The method name.
     * @param values The method values.
     * @throws IllegalStateException If can't invoke method.
     */
    public void updateCustomization(SpigotNPC spigotNpc,
                                    String name,
                                    String[] values) {
        ...
        try {
            ...
            method.invoke(spigotNpc.getBukkitEntity(), arrayToPrimitive(values, method));
            // update new customization for the npc
            spigotNpc.updateMetadata(spigotNpc.getViewers());
